...
function decomposeTo2NF(name, allAttrs, candidateKeys, fds) {
  ...
  for (const k of Object.keys(groups)) {
    ...
    // Key fix: compute closure of partial key directly to capture ALL transitivity 
    // dependent on it, e.g. CourseID -> CourseName, InstructorID -> InstructorName
    const { closure } = computeClosure(g.lhs, fds);
    const rAttrs = allAttrs.filter(a => closure.has(a) || g.lhs.includes(a));
    
    const rFDs = restrictFDs(fds, rAttrs);
    relations.push({ name: `R${idx++}`, attrs: rAttrs, fds: rFDs, note: `Partial dep: {${g.lhs.join(',')}} → {${[...g.rhs].join(',')}}` });
    
    // Only remove non-prime attributes that were extracted
    rAttrs.forEach(a => { if(!prime.has(a)) removedAttrs.add(a) });
  }
...
function decomposeTo3NF(relations) {
  ...
  for (const rel of relations) {
    ...
    for (const k of Object.keys(groups)) {
      ...
      // Similarly fix 3NF extraction to compute full closure of the LHS
      const { closure } = computeClosure(g.lhs, rel.fds);
      const rAttrs = rel.attrs.filter(a => closure.has(a) || g.lhs.includes(a));
      
      const rFDs = restrictFDs(rel.fds, rAttrs);
      result.push({ name: `R${idx++}`, attrs: rAttrs, fds: rFDs, note: `Transitive: {${g.lhs.join(',')}} → {${[...g.rhs].join(',')}}` });
      
      rAttrs.forEach(a => { if(!g.lhs.includes(a)) removedAttrs.add(a) });
    }
...
function decomposeToBCNF(relations, allOriginalAttrs, allOriginalFDs) {
  const result = [];
  function processRel(rel) {
    const attrs = rel.attrs;
    const fds = rel.fds;
    // We check violatons against internal FDs
    
    for (const fd of fds) {
      if (!fd.rhs.some(a => attrs.includes(a))) continue;
      
      // BCNF violation: if LHS is not a superkey of the ENTIRE overarching relation
      // NOT just this subset relation
      if (setSubset(new Set(fd.lhs), new Set(attrs)) && !isSuperkey(fd.lhs, allOriginalAttrs, allOriginalFDs)) {
        // violation
        const r1Attrs = [...new Set([...fd.lhs, ...fd.rhs])];
        const r2Attrs = attrs.filter(a => !fd.rhs.includes(a) || fd.lhs.includes(a));
        
        // Guarantee progress: if either decomposed relation has the same size as attrs, no real decomposition happened.
        if (new Set(r1Attrs).size >= attrs.length || new Set(r2Attrs).size >= attrs.length) continue;
        
        const r1FDs = restrictFDs(allOriginalFDs, r1Attrs);
        const r2FDs = restrictFDs(allOriginalFDs, r2Attrs);
