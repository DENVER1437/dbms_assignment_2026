...
function runNormalization(){
  ...
  const relationsBCNF=decomposeToBCNF(relations3NF, schema.attrs, fds);
...
// ═══════════════════
// LOSSLESS JOIN CHASE ALGORITHM + DEPENDENCY PRESERVATION
// ═══════════════════
function checkLosslessJoin(relations,originalAttrs,fds){
  if (relations.length === 1) {
    return { lossless: true, reason: 'Single relation — trivially lossless.' };
  }

  if (relations.length === 2) {
    const r1 = new Set(relations[0].attrs), r2 = new Set(relations[1].attrs);
    const inter = [...r1].filter(a => r2.has(a));
    if (!inter.length) return { lossless: false, reason: `R1 ∩ R2 = ∅<br>No shared attributes — join is a Cartesian product (lossy).` };
    
    // Check if intersection determines R1 or R2
    const { closure } = computeClosure(inter, fds);
    const r1Sub = [...r1].every(a => closure.has(a));
    const r2Sub = [...r2].every(a => closure.has(a));
    
    if (r1Sub || r2Sub) {
      return { lossless: true, reason: `${relations[0].name} ∩ ${relations[1].name} = {${inter.join(', ')}}<br>Intersection is a superkey for ${r1Sub ? relations[0].name : relations[1].name} ✓` };
    }
    return { lossless: false, reason: `${relations[0].name} ∩ ${relations[1].name} = {${inter.join(', ')}}<br>Intersection is NOT a superkey for either relation.` };
  }

  // Chase Algorithm for 3+ relations
  // Initialize tableau
  const rows = relations.map(r => new Set(r.attrs));
  const table = []; // table[row][col]
  const colIndex = {};
  originalAttrs.forEach((attr, i) => colIndex[attr] = i);
  
  for (let i = 0; i < relations.length; i++) {
    const rowObj = {};
    originalAttrs.forEach(attr => {
      rowObj[attr] = rows[i].has(attr) ? `a` : `b_${i}`;
    });
    table.push(rowObj);
  }
  ...
  while (changed) {
    changed = false;
    for (const fd of fds) {
      // Find rows that match on LHS
      for (let i = 0; i < table.length; i++) {
        for (let j = i + 1; j < table.length; j++) {
          const matchLHS = fd.lhs.every(a => table[i][a] === table[j][a]);
          if (matchLHS) {
            // Equate RHS
            for (const rAttr of fd.rhs) {
              const v1 = table[i][rAttr];
              const v2 = table[j][rAttr];
              if (v1 !== v2) {
                let replaceWith = v1;
                if (v1 === 'a') replaceWith = 'a';
                else if (v2 === 'a') replaceWith = 'a';
                else if (parseInt(v1.split('_')[1]||99) < parseInt(v2.split('_')[1]||99)) replaceWith = v1;
                else replaceWith = v2;
                
                const toReplace = v1 === replaceWith ? v2 : v1;
                
                // apply replacement everywhere in table
                for (let r = 0; r < table.length; r++) {
                  if (table[r][rAttr] === toReplace) {
                     table[r][rAttr] = replaceWith;
                     changed = true;
                  }
                }
              }
  ...
  // Check if any row has all 'a'
  const isLossless = table.some(row => originalAttrs.every(attr => row[attr] === 'a'));
  
  // Format intersections for purely visual reasoning in UI (user wants exact overlaps)
  const intersectReasons = [];
  for (let i = 0; i < relations.length; i++) {
    for (let j = i + 1; j < relations.length; j++) {
      const p1 = relations[i].attrs, p2 = relations[j].attrs;
      const sh = p1.filter(a => p2.includes(a));
      if (sh.length) intersectReasons.push(`${relations[i].name} ∩ ${relations[j].name} = {${sh.join(',')}}`);
      else intersectReasons.push(`${relations[i].name} ∩ ${relations[j].name} = ∅`);
    }
  }

  const reasonHTML = `${isLossless ? 'Chase Algorithm verified row of all distinguished variables (a).' : 'Chase Algorithm could not produce a row of all distinguished variables.'}<br><br><span style="font-size:0.75rem; color:var(--text3); font-family:var(--mono)">${intersectReasons.join('<br>')}</span>`;
  
  return { lossless: isLossless, reason: reasonHTML };
}

// ═══════════════════
// TRUE DEPENDENCY PRESERVATION ALGORITHM
// ═══════════════════
/**
 * Mathematically correct dependency preservation check based on standard DBMS theory.
 * Computes attribute closure over the UNION of all FDs projected on the decomposed relations,
 * preserving transitive inference across tables.
 */
function checkDependencyPreservation(relations, originalFDs) {
  ...
  // Validation
  if (!originalFDs || !Array.isArray(originalFDs)) return { preserved, lost };

  for (const fd of originalFDs) {
    let z = new Set(fd.lhs);
    let changed = true;
    
    // Compute the closure of X (the determinant) under the combined projected FDs (F_1 U F_2 ...)
    // Standard Dependency Preservation Algorithm
    while (changed) {
      changed = false;
      for (const rel of relations) {
        if (!rel.attrs || !Array.isArray(rel.attrs)) continue;
        ...
        // Compute closure of (Z_int_R_i) using ONLY the FDs projected onto R_i (which is rel.fds)
        const riFDs = rel.fds || [];
        const { closure } = computeClosure(z_int_ri, riFDs);
        
        // (Closure intersect R_i) added to Z
    ...
    // Dependency is preserved if the closure Z contains all attributes of Y
    const isPreserved = fd.rhs.every(a => z.has(a));
    
    if (isPreserved) {
      // Determine if it was trivially fully contained in one relation, or "Derived" transitively
      const strictlyContained = relations.some(r => {
        const rAttrs = new Set(r.attrs);
        return [...fd.lhs, ...fd.rhs].every(a => rAttrs.has(a));
      });
      preserved.push({ fd, derived: !strictlyContained });
    } else {
      lost.push(fd);
    }
  }
...
function buildLosslessCard(relationsBCNF,originalAttrs,originalFDs,delay){
  ...
  if(depPres.preserved.length)html+=`<div style="margin-bottom:.6rem"><span style="font-size:.75rem;color:var(--green);font-weight:600">✔ Preserved Dependencies (${depPres.preserved.length})</span><div style="margin-top:.3rem">${depPres.preserved.map(p=>`<span class="schema-box" style="margin:.2rem;display:inline-flex;align-items:center;gap:.3rem;${p.derived?'border-color:var(--gold);color:var(--gold)':''}"><span style="letter-spacing:1px">${p.fd.lhs.join(',')}</span><span class="arrow-label">→</span><span style="letter-spacing:1px">${p.fd.rhs.join(',')}</span>${p.derived?'<span style="font-size:0.65rem;text-transform:uppercase;margin-left:4px">(Derived)</span>':''}</span>`).join('')}</div></div>`;
  if(depPres.lost.length)html+=`<div><span style="font-size:.75rem;color:var(--red);font-weight:600">❌ Not Preserved Dependencies (${depPres.lost.length})</span><div style="margin-top:.3rem">${depPres.lost.map(fd=>fdStr(fd)).join('')}</div><p style="font-size:.8rem;color:var(--text3);margin-top:.5rem">⚠ Note: BCNF decomposition does not always preserve all FDs. This is a known trade-off vs 3NF which always preserves FDs.</p></div>`;
...
function generatePrintReport(){
  ...
  const relations3NF=decomposeTo3NF(relations2NF);
  const relationsBCNF=decomposeToBCNF(relations3NF, schema.attrs, _globalFDs);
  const{minimal}=computeMinimalCover(_globalFDs,schema.attrs);
  const lossless=checkLosslessJoin(relationsBCNF,schema.attrs,_globalFDs);
  const depPres=checkDependencyPreservation(relationsBCNF,_globalFDs);
  let sqlText='';
...
<div class="section pagebreak">
  ...
  ${depPres.preserved.length?`<p style="font-size:10pt;color:#065f46;font-weight:600;margin-bottom:.35rem;">✔ Preserved Dependencies (${depPres.preserved.length})</p><div class="chips">${depPres.preserved.map(p=>`<span class="fd" ${p.derived?'style="border-color:#d97706;color:#d97706;"':''}>${p.fd.lhs.join(',')}<span class="fd-arrow" ${p.derived?'style="color:#d97706;"':''}>→</span>${p.fd.rhs.join(',')}${p.derived?' <span style="font-size:8pt;opacity:0.8;">(Derived)</span>':''}</span>`).join('')}</div>`:``}
  ${depPres.lost.length?`<p style="font-size:10pt;color:#92400e;font-weight:600;margin:.75rem 0 .35rem;">❌ Not Preserved Dependencies (${depPres.lost.length}) — Known BCNF trade-off</p><div class="chips">${depPres.lost.map(fd=>`<span class="fd" style="border-color:#fca5a5;color:#dc2626;">${fd.lhs.join(',')}<span class="fd-arrow" style="color:#dc2626;">→</span>${fd.rhs.join(',')}</span>`).join('')}</div><p style="font-size:9.5pt;color:#92400e;margin-top:.5rem;">Note: BCNF does not always preserve all FDs. 3NF decomposition always preserves FDs but may not achieve BCNF.</p>`:`<div class="alert alert-ok">✓ All original FDs are preserved in the decomposed relations.</div>`}
</div>
<div class="section">
