...
function runNormalization(){
  ...
  // Stats bar
  setTimeout(()=>{
    const statsBar=buildStatsBar(schema.attrs,candidateKeys,relations2NF,relations3NF,relationsBCNF,partials,allTransitive);
    document.getElementById('output-area').prepend(statsBar);
  },0);

  // Render cards with staggered delay (10 cards)
  const cards=[
    ()=>buildParsedCard(schema,fds,0),
    ()=>buildClosureCard(schema.attrs,fds,180),
    ()=>buildKeysCard(schema.attrs,candidateKeys,fds,360),
    ()=>buildMinimalCoverCard(schema.attrs,fds,540),
    ()=>build1NFCard(schema,720),
    ()=>build2NFCard(schema,candidateKeys,fds,relations2NF,partials,900),
    ()=>build3NFCard(relations2NF,relations3NF,1080),
    ()=>buildBCNFCard(relations3NF,relationsBCNF,1260),
    ()=>buildLosslessCard(relationsBCNF,schema.attrs,fds,1440),
    ()=>buildSummaryCard(schema, candidateKeys, partials, relations2NF, relations3NF, relationsBCNF, allTransitive, 1620),
  ];

  // Button pulse + progress bar
  const runBtn=document.getElementById('run-btn');
  runBtn.classList.add('btn-processing');
  runBtn.textContent='⟳ Processing...';
  const progWrap=document.getElementById('progress-wrap');
  progWrap.style.display='block';
  document.getElementById('progress-bar').style.width='0%';
  document.getElementById('progress-label').textContent='Step 0 of '+cards.length;
...
function computeMinimalCover(fds,allAttrs){
  const steps=[];
  let current=[];
  fds.forEach(fd=>{fd.rhs.forEach(b=>current.push({lhs:[...fd.lhs],rhs:[b]}));});
  steps.push({title:'Step 1: Decompose RHS to singletons',fds:current.map(f=>({...f}))});
  current=current.map(fd=>{
    if(fd.lhs.length<=1)return fd;
    const newLhs=[...fd.lhs];
    for(let i=newLhs.length-1;i>=0;i--){
      const reduced=newLhs.filter((_,j)=>j!==i);
      const{closure}=computeClosure(reduced,current);
      if(closure.has(fd.rhs[0]))newLhs.splice(i,1);
    }
    return{lhs:newLhs,rhs:fd.rhs};
  });
  steps.push({title:'Step 2: Remove extraneous LHS attributes',fds:current.map(f=>({...f}))});
  const minimal=[];
  current.forEach((fd,i)=>{
    const without=current.filter((_,j)=>j!==i);
    const{closure}=computeClosure(fd.lhs,without);
    if(!closure.has(fd.rhs[0]))minimal.push(fd);
  });
  steps.push({title:'Step 3: Remove redundant FDs',fds:minimal.map(f=>({...f}))});
  return{minimal,steps};
}

function buildMinimalCoverCard(attrs,fds,delay){
  const{minimal,steps}=computeMinimalCover(fds,attrs);
  let html=`<p style="color:var(--text2);font-size:.875rem;margin-bottom:1rem">The <strong>Minimal Cover</strong> (canonical cover) is the smallest equivalent set of FDs with no redundancy. Used as the basis for 3NF synthesis.</p>`;
  steps.forEach(step=>{
    html+=`<div style="margin-bottom:1rem"><div style="font-size:.78rem;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.07em;margin-bottom:.4rem">${step.title}</div><div>${step.fds.length?step.fds.map(fd=>fdStr(fd)).join(''):'<span style="color:var(--text3);font-size:.85rem">No FDs remain</span>'}</div></div>`;
  });
  ...
  return makeCard('card-minimal','badge-minimal','MINIMAL COVER','Canonical Cover of FDs',html,'card-minimal',delay);
}
...
function checkDependencyPreservation(relations,originalFDs){
  ...
  originalFDs.forEach(fd=>{
    ...
    if(inRel)preserved.push(fd);else lost.push(fd);
  });
...
function buildLosslessCard(relationsBCNF,originalAttrs,originalFDs,delay){
  ...
  if(depPres.preserved.length)html+=`<div style="margin-bottom:.6rem"><span style="font-size:.75rem;color:var(--green);font-weight:600">PRESERVED (${depPres.preserved.length})</span><div style="margin-top:.3rem">${depPres.preserved.map(fd=>fdStr(fd)).join('')}</div></div>`;
...
<div class="section pagebreak">
  ...
  ${depPres.preserved.length?`<p style="font-size:10pt;color:#065f46;font-weight:600;margin-bottom:.35rem;">Preserved FDs (${depPres.preserved.length})</p><div class="chips">${depPres.preserved.map(fd=>`<span class="fd">${fd.lhs.join(',')}<span class="fd-arrow">→</span>${fd.rhs.join(',')}</span>`).join('')}</div>`:``}
